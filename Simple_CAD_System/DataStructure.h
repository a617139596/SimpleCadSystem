...
public:
	...
	FaceData() {
		...
		normalDir = glm::vec3(0.0f, 0.0f, 0.0f);
	}
};
...
public:
	...
	void ListToArray();
	void PrintInfo();
	void OutputData(const char name[]);
};
BrepObject brep;
...
void BrepObject::ListToArray() {
	...
	for (int i = 0; i < faceNum; i++) {
		...
		temp.solidIndex = 0;
		temp.normalDir = faceList[i]->normalDir;
...
void BrepObject::OutputData(const char name[]) {
	std::ofstream brepFile(name);
	brepFile << "BRP\n" << vertexNum << ' ' << loopNum << ' ' << faceNum << ' ' << solidNum << std::endl;
	for (int i = 0; i < vertexArray.size(); i++) {
		brepFile << vertexArray[i] << ' ';
		if ((i + 1) % 3 == 0)
			brepFile << std::endl;
	}
	for (int i = 0; i < loopArray.size(); i++) {
		brepFile << loopArray[i].size();
		for (int j = 0; j < loopArray[i].size(); j++)
			brepFile << ' ' << loopArray[i][j];
		brepFile << std::endl;
	}
	for (int i = 0; i < faceArray.size(); i++) {
		brepFile << faceArray[i].outerLoopIndex << ' ' << faceArray[i].innerLoopNum;
		for (int j = 0; j < faceArray[i].innerLoopIndexArray.size(); j++)
			brepFile << ' ' << faceArray[i].innerLoopIndexArray[j];
		brepFile << ' ' << faceArray[i].solidIndex << std::endl;
	}
}
